...
void ForceProtect( gentity_t *self )
{
	...
	//VISION:
	if (self->client->ps.fd.forcePowersActive & (1 << FP_RAGE) && !(v_allowForceCombo.uinteger & 2))
	{
		WP_ForcePowerStop( self, FP_RAGE );
	}
	if (self->client->ps.fd.forcePowersActive & (1 << FP_ABSORB) && !(v_allowForceCombo.uinteger & 1))
	{
...
void DoGripAction(gentity_t *self, forcePowers_t forcePower)
{
	...
	if (gripLevel == FORCE_LEVEL_3)
	{
		...
		if (gripEnt->client->ps.forceGripMoveInterval < level.time)
		{
			...
			//VISION:
			gripEnt->client->ps.forceGripMoveInterval = level.time + v_forcegripUpdaterate.integer; //only update velocity every 300ms, so as to avoid heavy bandwidth usage
